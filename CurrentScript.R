...
# select adult only
DEMO18TO80<-DEMO[DEMO$RIDAGEYR>=18 & DEMO$RIDAGEYR <= 80,]



# Keeping merging
DEMO2<- left_join(DEMO18TO80, SMQ, by="SEQN")
...
# Select varibles
DEMO5<- DEMO2 %>% select(SEQN, RIAGENDR, RIDAGEYR, DMDEDUC3,DMDEDUC2 , RIDRETH1, SMQ050Q, SMQ050U,SMD055 ,INDFMPIR, BMXBMI,OHAROCGP, OHAROCDT,OHQ030 ,SMQ040, SMQ020, SMD650, SMD030, OHAREC, OHXRCAR, OHXRCARO, LBXBPB, LBXCOT, LBDCOTLC, OHQ850, ALQ130, HIQ011, DIQ010)  
...
# Table one
library(tableone)
numvar<- c("RIDAGEYR", "SMD030", "LBXBPB", "LBXCOT")
...
catvar2<-c("RIAGENDR", "RIDRETH1", "SMQTYPE", "LBDCOTLC","GUM_DECAYTEETH", "OHXCART", "SMQ040","SMQ020" ,"LAST_DENTAL_SESSION", "EDULEVEL", "OHQ850", "BMXBMI", "INDFMPIR", "ALQ130", "HIQ011", "DIQ010")
...
tab2 <- CreateTableOne( strata = "OH" , data = DEMO5, factorVars = catvar2)
print(tab2, formatOptions = list(big.mark = ","), nonnormal =numvar)
...
f2<-glm(p2~SESSION+RIDAGEYR+RACE+SEX+SMOKER+DRINK+INSURANCE+PIR+BMI+LBXBPB+LBXCOT+OHHIST+EDU, family=binomial(), data = DEMO5)
summary(f2)
...
forestplot(labeltext = as.matrix(dt[,1:5]),##研究下刻度
           ...
           is.summary = c(F,T,T,F,T,F,F,F,F,T,F,F,T,F,F,T,F,T,F,F,T,T,T,F,T,F,F,T,F,T,F),
...
#/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
# 分层细化：前吸烟者中不同程度 （年为单位）戒烟者的口腔(需要考虑上次抽烟到现在什么时候SMD055) 
TEST1<- DEMO5 %>% filter(SMQTYPE=="Former Smoker")
TEST1[ , 'QUITTIME'] <- NA
for(i in 1:nrow(TEST1)) { 
  if (!is.na(TEST1[i,"SMQ050U"]) && TEST1[i,"SMQ050U"]==2) {
    TEST1[i,"QUITTIME"] <- TEST1[i, "SMQ050Q"] * 0.0191781
  }
  else if (!is.na(TEST1[i,"SMQ050U"]) &&TEST1[i,"SMQ050U"]==3)
  {
    TEST1[i,"QUITTIME"] <- TEST1[i, "SMQ050Q"] * (1/12)
  }
...
# SMQTYPE2: 戒烟少于5年人是reference， 戒烟超过15年
TEST1$SMQTYPE2[TEST1$QUITTIME<=5]<-"Quit smoking less than 5 years"
TEST1$SMQTYPE2[TEST1$QUITTIME>5 & TEST1$QUITTIME<=10]<-"Quit smoking 5 years to 10 years"
#TEST1$SMQTYPE2[TEST1$QUITTIME>10 ]<-"Quit smoking more than 10 years"##
TEST1$SMQTYPE2[TEST1$QUITTIME>10 & TEST1$QUITTIME<15]<-"Quit smoking 10 to 15 years"
TEST1$SMQTYPE2[TEST1$QUITTIME>=15]<-"Quit smoking more than 15 years"
...
# barplot!!
TEST1$X<-relevel(SMOKER, ref="Quit smoking less than 5 years")
...
# barplot 百分比！
SMOKER_OH <- table(TEST1$OH,TEST1$X)
...
survivalGender <- table(TEST1$OH,TEST1$X)
...
# table one
numvar<- c("RIDAGEYR","INDFMPIR", "BMXBMI", "SMD030", "LBXBPB", "LBXCOT")
catvar<-c("RIAGENDR", "RIDRETH1", "OH", "LBDCOTLC","GUM_DECAYTEETH", "OHXCART", "SMQ040", "SMQ020", "LAST_DENTAL_SESSION", "EDULEVEL", "OHQ850")
catvar2<-c("RIAGENDR", "RIDRETH1", "SMQTYPE2", "LBDCOTLC","GUM_DECAYTEETH", "OHXCART", "SMQ040","SMQ020" ,"LAST_DENTAL_SESSION", "EDULEVEL", "OHQ850")
...
# 按照OH分层(需要注意那些需要展示和变量分类确认)
tab2 <- CreateTableOne( strata = "OH" , data = TEST1, factorVars = catvar2)
print(tab2, formatOptions = list(big.mark = ","), nonnormal =numvar)
...
#SMOKER1<-as.factor(TEST1$SMQTYPE2)
EDU1<-as.factor(TEST1$EDULEVEL)
SMOKER<-as.factor(TEST1$SMQTYPE2)
ROOT1<-as.factor(TEST1$OHXCART)
TEST1$X<-relevel(SMOKER, ref="Quit smoking less than 5 years")
f2<-glm(p3~SESSION1+RIDAGEYR+RACE1+SEX1+X+INDFMPIR+BMXBMI+LBXBPB+LBXCOT+OHHIST1+EDU1, family=binomial(), data = TEST1)
summary(f2)
A<-data.frame(exp(confint(f2)))#CI!!!!!
B<-data.frame(exp(f2$coef))#OR
with(summary(f2), 1 - deviance/null.deviance)
...
#?????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
# 分层细化：前吸烟者中不同程度 （年为单位）戒烟者的口腔(需要考虑上次抽烟到现在什么时候SMD055 ， 2011-2016only) 
TEST1<- DEMO5 %>% filter(SMQTYPE=="Former Smoker")
TEST1<- TEST1 %>% filter(!is.na(SMD055))
#TEST1[ , 'QUITTIME'] <- NA
TEST1$QUITTIME<-TEST1$RIDAGEYR-TEST1$SMD055
TEST1<- TEST1 %>% filter(QUITTIME>=0)
